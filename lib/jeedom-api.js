...
JeedomClient.prototype.getRooms = function() {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"sync","params":{"plugin":"mobile","apikey":"' + that.apikey + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200) {
				resolve(json.result.objects);
			} else {
				reject(err, response);
			}
		});
