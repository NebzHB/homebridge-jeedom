...
function JeedomClient(url, apikey) {
	this.apikey = apikey;
...
JeedomClient.prototype.getRooms = function() {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"object::all","params":{"apikey":"' + that.apikey + '"}}'
			}
...
JeedomClient.prototype.getDevices = function() {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"eqLogic::all","params":{"apikey":"' + that.apikey + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200)
				resolve(json.result);
...
JeedomClient.prototype.getScenes = function() {
	var that = this;
	var p = new Promise(function(resolve, reject) {
		var url = that.url;
		var request_id = Math.floor(Math.random() * 1000);
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"scenario::all","params":{"apikey":"' + that.apikey + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200)
				resolve(json.result);
...
JeedomClient.prototype.getDeviceProperties = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"eqLogic::byId","params":{"apikey":"' + that.apikey + '","id":"' + ID + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200) {
				resolve(json.result);
...
JeedomClient.prototype.getDeviceCmd = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"cmd::byEqLogicId","params":{"apikey":"' + that.apikey + '","eqLogic_id":"' + ID + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200) {
				resolve(json.result);
...
JeedomClient.prototype.refreshStates = function(lastPoll) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			...
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"event::changes","params":{"apikey":"' + that.apikey + '","longPolling":"30","datetime" : "' + lastPoll + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200) {
				resolve(json.result);
