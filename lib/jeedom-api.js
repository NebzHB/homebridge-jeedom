...
JeedomClient.prototype.updateModelInfo = function(ID,value) {
	...
	for (var c in that._cachedModel.cmds) {
		...
		if(cmd.id == ID && cmd.type=='info') {
			...
			var eq = that.getDeviceProperties(cmd.eqLogic_id)
			that.log('info','[Maj reçue de Jeedom] commande:'+ID+' value:'+value);
			that.log('info','[[Pas une commande INFO ('+cmd.type+') '+eq.name+'>'+cmd.name+'('+cmd.generic_type+') '+value+' dans ' + JSON.stringify(cmd).replace('\n',''));
			return null;
		}
	}
...
JeedomClient.prototype.ParseGenericType = function(EqLogic, cmds) {
		...
		if (isset(cmds[i].generic_type)) {
			if (cmds[i].generic_type !== null) {
				...
				case "GB_TOGGLE" :
					...
					result_cmd.GarageDoor[i].toggle = cmds[i];
					break;
				/***************** LOCK ***********************/
				case 'LOCK_STATE' :
				...
				case 'DOORBELL_STATE' :
					if (result_cmd.DoorBell == undefined) {
						result_cmd.DoorBell = [];
					}
					if (result_cmd.DoorBell[i] == undefined) {
						result_cmd.DoorBell[i] = {};
					}
					result_cmd.DoorBell[i].state = cmds[i];
					break;
				/***************** FLAP ***********************/
				case 'FLAP_STATE' :
	...
	if (isset(result_cmd.light)) {
		...
		if (isset(result_cmd.GarageDoor.state) && (isset(result_cmd.GarageDoor.on) || isset(result_cmd.GarageDoor.off) || isset(result_cmd.GarageDoor.toggle))) {
