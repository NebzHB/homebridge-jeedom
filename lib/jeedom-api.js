...
function JeedomClient(url, apikey) {
	...
	//this.plateform = module.parent; // need to find how to do
	...
	return new Promise(function(resolve, reject) {
		...
		request.post(url, {
			...
			if (!err && response.statusCode == 200) {
				...
				//module.parent.log(that._cachedModel);
...
JeedomClient.prototype.updateModelInfo = function(ID,value) {
	...
	for (var c in that._cachedModel.cmds) {
		...
		{
			console.log("[[have to mod from "+cmd.currentValue+" to "+value+" in " + JSON.stringify(cmd).replace("\n",""));
			cmd.currentValue=value;
			console.log("[[done:" + JSON.stringify(cmd).replace("\n",""));
...
module.exports.createClient = function(url, apikey) {
	return new JeedomClient(url, apikey);
};
