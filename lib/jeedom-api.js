		...
		if (isset(cmds[i].generic_type)) {
			if (cmds[i].generic_type !== null) {
				...
				case 'ALARM_STATE' :
					...
					result_cmd.services.alarm.state = cmds[i];
					break;
				case 'ALARM_MODE' :
					if (result_cmd.services.alarm == undefined) {
						result_cmd.services.alarm = {};
					}
					result_cmd.services.alarm.mode = cmds[i];
					break;
				case 'ALARM_ENABLE_STATE' :
					if (result_cmd.services.alarm == undefined) {
						result_cmd.services.alarm = {};
					}
					result_cmd.services.alarm.enable_state = cmds[i];
					break;
				case 'ALARM_ARMED' :
					if (result_cmd.services.alarm == undefined) {
						result_cmd.services.alarm = {};
					}
					result_cmd.services.alarm.armed = cmds[i];
					break;
				case 'ALARM_RELEASED' :
					if (result_cmd.services.alarm == undefined) {
						result_cmd.services.alarm = {};
					}
					result_cmd.services.alarm.released = cmds[i];
					break;
				case 'ALARM_SET_MODE' :
					if (result_cmd.services.alarm == undefined) {
						result_cmd.services.alarm = {};
					}
					result_cmd.services.alarm.set_mode = cmds[i];
					break;
				/***************** GENERIC ***********************/
				case 'OPENING_WINDOW' :
				case 'OPENING' :
					...
					break;
				/*case 'OPENING_WINDOW' :
					if (result_cmd.services.openwindow == undefined) {
						result_cmd.services.openwindow = [];
					}
					if (result_cmd.services.openwindow[i] == undefined) {
						result_cmd.services.openwindow[i] = {};
					}
					result_cmd.services.openwindow[i].openwindow = cmds[i];
					break;*/
				case 'BATTERY' :
				...
				case 'BRIGHTNESS' :
					...
					break;
				case 'SECURITY_STATE' :
					if (result_cmd.services.security_state == undefined) {
						result_cmd.services.security_state = [];
					}
					if (result_cmd.services.security_state[i] == undefined) {
						result_cmd.services.security_state[i] = {};
					}
					result_cmd.services.security_state[i].security_state = cmds[i];
					break;
				case 'SMOKE' :
