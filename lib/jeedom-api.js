...
JeedomClient.prototype.getModel = function() {
	...
	return axios.post(url, 
	{
		...
		params:{
			...
			apikey:that.apikey,
			session:true,
			sess_id:that.sess_id
		}
	}).then(result => {
		if(!result.data) {throw new Error("JSON reçu de Jeedom invalide, vérifiez le log API de Jeedom, reçu :"+JSON.stringify(result));}
		if(!result.data.result && result.data.error) {
			throw new Error(result.data.error.message);
		} else {
			if(result.data.sess_id !== undefined) {
				that.sess_id = result.data.sess_id;
			} else {
				that.sess_id = "";
			}
	...
	return axios.post(url, 
	{
		...
		if(!result.data.result && result.data.error) {
			...
			if(result.data.result != 'ok') {
				...
				throw new Error("EqLogic "+ID+" n'existe pas ou pas envoyé à homebridge");
			}
		}
	});
...
JeedomClient.prototype.ParseGenericType = function(EqLogic, cmds) {
	...
	result_cmd = EqLogic;
	result_cmd.services = {};
	result_cmd.numSwitches=0;
	result_cmd.numDetector=0;
	for (var i in cmds) {
		if (isset(cmds[i].generic_type)) {
			if (cmds[i].generic_type) {
				...
				case 'LIGHT_BRIGHTNESS' :
					...
					result_cmd.services.light[i].brightness = cmds[i];
					break;
				...
				case 'WEATHER_RAIN' :
					...
					result_cmd.services.weather[i].rain = cmds[i];
					break;
				case 'WEATHER_SNOW' :
					if (!result_cmd.services.weather) {
						result_cmd.services.weather = [];
					...
					result_cmd.services.weather[i].snow = cmds[i];
					break;	
				case 'WEATHER_TEMPERATURE_MIN' :
					...
					result_cmd.services.weather[i].temperature_min = cmds[i];
					break;
				/** *************** SIREN ***********************/
				case 'SIREN_STATE' :
				...
				case 'AIRQUALITY_CUSTOM' :
					if (!result_cmd.services.AirQualityCustom) {
						result_cmd.services.AirQualityCustom = [];
					}
					if (!result_cmd.services.AirQualityCustom[i]) {
						result_cmd.services.AirQualityCustom[i] = {};
					}
					result_cmd.services.AirQualityCustom[i].Index = cmds[i];
					break;
				case 'NOISE' :
