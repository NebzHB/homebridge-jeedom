...
JeedomClient.prototype.getModel = function() {
	...
	return new Promise(function(resolve, reject) {
		...
		request.post(url, {
			...
			that.log('debug',"err:",JSON.stringify(err).replace("\n",''));	
			that.log('debug',"response:",JSON.stringify(response).replace("\n",''));
			if (!err && response.statusCode == 200) {
				if(json) {
					that.log('debug',"json:",JSON.stringify(json).replace("\n",''));
					if(json.result != undefined) {
						that.log('debug',"json.result:",JSON.stringify(json.result).replace("\n",''));
						if(json.result.config != undefined)
							that.log('debug',"DateTime result:",json.result.config.datetime);
						that._cachedModel=json.result;
					}
				}
				else {
					that.log('debug',"json:",json);
				}
				if(that._cachedModel.config != undefined)
