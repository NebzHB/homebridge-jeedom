...
JeedomClient.prototype.getModel = function() {
	...
	return new Promise(function(resolve, reject) {
		...
		request.post(url, {
			...
			gzip : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"sync_homebridge","params":{"plugin":"'+that.myPlugin+'","apikey":"' + that.apikey + '"}}'
			}
...
JeedomClient.prototype.getDeviceProperties = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"getEql","params":{"plugin":"'+that.myPlugin+'","apikey":"' + that.apikey + '","id":"' + ID + '"}}'
			}
		}, function(err, response, json) {
...
JeedomClient.prototype.getScenarioProperties = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"scenario::byId","params":{"apikey":"' + that.apikey + '","id":"' + ID + '"}}'
			}
		}, function(err, response, json) {
			if (!err && response.statusCode == 200 && json.result != undefined) {
				resolve(json.result);
	...
	for (var i in cmds) {
		if (isset(cmds[i].generic_type)) {
			if (cmds[i].generic_type) {
				...
				case 'DOORBELL_STATE' :
					if (!result_cmd.services.DoorBell) {
						result_cmd.services.DoorBell = [];
					}
					if (!result_cmd.services.DoorBell[i]) {
				...
				case 'SWITCH_STATELESS_SINGLE' :
					...
					result_cmd.services.StatelessSwitch[i].Single = cmds[i];
					break;
				case 'SWITCH_STATELESS_DOUBLE' :
					...
					result_cmd.services.StatelessSwitch[i].Double = cmds[i];
					break;
				case 'SWITCH_STATELESS_LONG' :
					...
					result_cmd.services.StatelessSwitch[i].Long = cmds[i];
					break;
				/***************** FLAP ***********************/
				case 'FLAP_STATE' :
...
JeedomClient.prototype.executeScenarioAction = function(ID, action) {
	...
	return new Promise(function(resolve, reject) {
		...
		request.post(url, {
			json : true,
			form : {
				request : '{"jsonrpc":"2.0","id":"'+request_id+'","method":"scenario::changeState","params":{"apikey":"' + that.apikey + '","id":"' + ID + '","state":"' + action + '"}}'
			}
		}, function(err, response, json) {
			if (!err && (response.statusCode == 200 || response.statusCode == 202))
				resolve(json.result);
