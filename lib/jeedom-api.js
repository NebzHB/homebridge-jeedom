...
JeedomClient.prototype.getDeviceProperties = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		//console.log('eq1'+JSON.stringify(json.result));
      		if (!err && response.statusCode == 200){ 
	...
	//console.log('EQLogic in get DeviceProperties'+JSON.stringify(p));
	//return p;
}
JeedomClient.prototype.getDeviceCmd = function(ID) {
	...
	var p = new Promise(function(resolve, reject) {
		...
		//console.log('cmd1'+JSON.stringify(json));
      		if (!err && response.statusCode == 200){ 
        		//console.log('cmd1'+JSON.stringify(json.result));
			//return json.result;
			resolve(json);
        	}else{
        		reject(err, response);
