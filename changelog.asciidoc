...
Si vous aviez un pont Philips Hue v1, vous avez maintenant accès à HomeKit :)
    ...
    * Les types spécifiques à Homebridge : j'ai maintenant la possibilité d'avoir des types spécifiques pour homebridge, ceux-ci ne font pas partie du core (comme les types génériques) mais complètent ceux-ci. Il faut néanmoins les définir manuellement dans le plugin. (les types génériques restent utilisés principalement, ces types sont des compléments pour les types génériques qui n'existent pas)
    * Nouveaux types spécifiques à homebridge : 
    * Nouveaux types spécifique à homebridge : 
