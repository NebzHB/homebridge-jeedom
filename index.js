		...
		if (eqLogic.services.power || (eqLogic.services.power && eqLogic.services.consumption)) {
			eqLogic.services.power.forEach(function(cmd) {
				if (cmd.power) {
					...
					HBservice.controlService.eqID = eqLogic.id;
					HBservice.controlService.subtype = eqLogic.id + '-' + cmd.power.id + '-' + HBservice.controlService.subtype;
					HBservices.push(HBservice);
					HBservice = null;
				}
			});
		...
		if (eqLogic.services.speaker) {
			eqLogic.services.speaker.forEach(function(cmd) {
				if (cmd.mute) {
					...
					HBservice.controlService.eqID = eqLogic.id;
					HBservice.controlService.subtype = eqLogic.id + '-' + cmd.mute.id + '-' + HBservice.controlService.subtype;
					HBservices.push(HBservice);
				}
			});
			if(!HBservice) {
		...
		if (eqLogic.services.alarm) {
			eqLogic.services.alarm.forEach(function(cmd) {
				if(cmd.enable_state) {
					...
					HBservice.controlService.eqID = eqLogic.id;
					HBservice.controlService.subtype = eqLogic.id + '-' + cmd.enable_state.id + '-' + HBservice.controlService.subtype;
					HBservices.push(HBservice);
					HBservice = null;
				}
			});
